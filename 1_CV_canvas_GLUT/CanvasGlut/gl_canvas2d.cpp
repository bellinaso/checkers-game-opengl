...
float Colors[22][3] =
{
	...
	// --- Paleta: Jogo de Damas (Madeira e Carmesim) ---
	{0.957, 0.945, 0.918}, // 14 Fundo da Tela
	{0.173, 0.173, 0.173}, // 15 Texto / UI
	{0.902, 0.784, 0.651}, // 16 Casa Clara
	{0.557, 0.353, 0.212}, // 17 Casa Escura
	{0.851, 0.220, 0.220}, // 18 Peça P1 (Carmesim)
	{0.149, 0.149, 0.149}, // 19 Peça P2 (Carvão)
	{0.298, 0.686, 0.314}, // 20 Destaque Movimento
	{0.949, 0.788, 0.298}  // 21 Destaque Seleção
};
...
void CV::centeredText(float x, float y, const char* t)
{
	int tam = (int)strlen(t);
	CV::translate(-(tam / 2 * 10), 0);
	for (int c = 0; c < tam; c++)
	{
		glRasterPos2i(x + c * 10, y);
		glutBitmapCharacter(GLUT_BITMAP_8_BY_13, t[c]);
...
void CV::button(float x1, float y1, float x2, float y2, const char* t)
{
	CV::rect(x1, y1 - 10, x2, y2);

	CV::centeredText((x1 + (x2 - x1) / 2), y1, t);
}
